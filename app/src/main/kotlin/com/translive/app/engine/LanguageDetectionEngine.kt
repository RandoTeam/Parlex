...
class LanguageDetectionEngine @Inject constructor() {
    ...
    fun isLatinKrakozyabry(text: String): Boolean {
        if (text.length < 3) return false
        val words = text.split(Regex("""\s+""")).filter { it.length >= 2 }
        if (words.isEmpty()) return false

        val suspiciousPatterns = listOf(
            Regex("""[bB][lI1|]"""),       // 'Ы' -> bl / bI / b1 / b|
            Regex("""[nN][pP][uU]"""),     // 'ПРИ' -> npu
            Regex("""LLI|LLl|lLl"""),      // 'Ш' -> LLI
            Regex("""[I1][_\-][I1]"""),    // 'Д' / 'П' -> I_I / 1-1
            Regex("""[T|][I|][pP]"""),     // 'ПР' -> TIp
            Regex("""\b[bB][bB]\b"""),     // 'ВВ' -> BB
            Regex("""[cC][T][O0][nN]"""),  // 'СТОП' -> CTOn
            Regex("""[bB][bB][lI1]"""),    // 'ВЫ' -> Bbl
            Regex("""[A-Za-z]+[0-9]+[A-Za-z]+""") // mixed digits inside words from OCR
        )

        var suspiciousMatches = 0
        for (word in words) {
            if (suspiciousPatterns.any { it.containsMatchIn(word) }) {
                suspiciousMatches++
            }
        }

        return suspiciousMatches > 0 && (suspiciousMatches.toFloat() / words.size.toFloat()) >= 0.20f
    }
    ...
    fun detectByScript(text: String, fallback: Language = Language.ENGLISH): Language {
        ...
        return when (dominant) {
            ...
            TextScript.THAI -> Language.THAI
            TextScript.BENGALI -> Language.BENGALI
            TextScript.TAMIL -> Language.TAMIL
            TextScript.TELUGU -> Language.TELUGU
            TextScript.BURMESE -> Language.BURMESE
            TextScript.KHMER -> Language.KHMER
    ...
    private suspend fun identifyWithMlKit(text: String): String? {
        val client = identifier ?: return null
    ...
    private fun languageHeuristicScore(
        ...
        var hintScore = words.count { it in hints } * 3
        var charScore = lowerText.count { it in distinctiveChars } * 4
        if (hints === VIETNAMESE_HINTS) {
            val vietnameseToneCount = lowerText.count { it in '\u1EA0'..'\u1EF9' }
            charScore += vietnameseToneCount * 6
        }
    ...
    private fun expectedScript(language: Language): TextScript =
        when (language.code) {
            ...
            "hi", "mr", "gu" -> TextScript.DEVANAGARI
            "ar", "fa", "ur", "ug" -> TextScript.ARABIC
            "he" -> TextScript.HEBREW
            "th" -> TextScript.THAI
            "bn" -> TextScript.BENGALI
            "ta" -> TextScript.TAMIL
            "te" -> TextScript.TELUGU
            "my" -> TextScript.BURMESE
            "km" -> TextScript.KHMER
    ...
    private fun scriptForChar(char: Char): TextScript {
        return when {
            char in '\u0400'..'\u052F' || char in '\u2DE0'..'\u2DFF' || char in '\uA640'..'\uA69F' -> TextScript.CYRILLIC
            ...
            char in '\u0E00'..'\u0E7F' -> TextScript.THAI
            char in '\u0980'..'\u09FF' -> TextScript.BENGALI
            char in '\u0B80'..'\u0BFF' -> TextScript.TAMIL
            char in '\u0C00'..'\u0C7F' -> TextScript.TELUGU
            char in '\u1000'..'\u109F' -> TextScript.BURMESE
            char in '\u1780'..'\u17FF' -> TextScript.KHMER
            else -> TextScript.OTHER
        }
    }

    private enum class TextScript {
        CYRILLIC, LATIN, CJK, DEVANAGARI, ARABIC, HEBREW, THAI, BENGALI, TAMIL, TELUGU, BURMESE, KHMER, OTHER
    }

    companion object {
        private const val TAG = "LanguageDetectionEngine"
        private const val MIN_DETECTION_CHARS = 2
        private const val MIN_SCRIPT_LETTERS = 1
        ...
        private val VIETNAMESE_HINTS = setOf("va", "la", "cua", "cho", "mot", "voi", "khong", "xin", "chao", "chuc", "cac", "ban", "ngay", "tot", "lanh")
